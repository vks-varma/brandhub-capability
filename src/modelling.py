...
def cfa(output_config, refresh_config, feat_eng_config, eq_sub_scale_merged_brand, req_cols, storage_options, refresh_type):
    # Activate automatic conversion
    pandas2ri.activate()
    ...
    req_cols_r_df = pandas2ri.py2rpy(req_cols)
    # Pass the config to R as individual values
    robjects.r.assign('ALL_CATEGORY_PILLARS', robjects.StrVector(feat_eng_config["cfa"]["pillars"]['all_category_pillars']))
    robjects.r.assign('BY_CATEGORY_PILLARS', robjects.StrVector(feat_eng_config["cfa"]["pillars"]['by_category_pillars']))
    robjects.r.assign('std_lv', feat_eng_config["cfa"]["std_lv"])
    robjects.r.assign('check_gradient', feat_eng_config["cfa"]["check_gradient"])
    ...
    robjects.r.assign('sample_seeds', robjects.IntVector(feat_eng_config["cfa"]["sample_seeds"]))
...
def run_model_type(model_type, feat_eng_config, refresh_config, X_train, y_train, X_test_hold, y_test_hold, train_x_all, train_y_all, dv, brand_name, category_name, pillar_name, temp_title):
    ...
    regressor = model_class(random_state=feat_eng_config["weights_model"]["hyperparameters"][model_type]["random_state"])
    search = GridSearchCV(regressor, param_grid, cv=feat_eng_config["weights_model"]["cross_validation_number"],
                          scoring=['r2', 'neg_mean_absolute_percentage_error'], refit='neg_mean_absolute_percentage_error')
    search.fit(X_train, y_train)

    print(f"The best hyperparameters for {model_type} are {search.best_params_}")
    ...
    if model_type == "RandomForest":
        ...
        rfe = RFECV(regressor, cv=feat_eng_config["weights_model"]["cross_validation_number"], scoring='neg_mean_absolute_percentage_error')
        rfe.fit(X_train, y_train)
        selected_features = list(np.array(features)[rfe.get_support()])
        print(selected_features)
        ...
        dict_result = regressor.get_booster().get_score(importance_type='gain')
        print(f"Feature importance for {model_type}", pd.DataFrame(dict_result.items(), columns=['Feature', 'gain']).sort_values(by='gain', ascending=False))

    explainer = shap.TreeExplainer(regressor)
    shap_values = explainer.shap_values(X_train)
    feat_df = pd.DataFrame(np.abs(pd.DataFrame(shap_values, columns=X_train.columns)).mean(), columns=['shap values'])
    print(f"{model_type} shap importance", feat_df.sort_values(by='shap values', ascending=False))
    ...
    results_all_model['R2_score_fold'] = cross_val_score(regressor, X_train, y_train, cv=feat_eng_config["weights_model"]["cross_validation_number"], scoring='r2').mean()
    results_all_model['MAPE_fold'] = cross_val_score(regressor, X_train, y_train, cv=feat_eng_config["weights_model"]["cross_validation_number"], scoring='neg_mean_absolute_percentage_error').mean() * -1
    results_all_model['R2_score_Hold_out'] = metrics.r2_score(y_test_hold, y_pred_test)
    results_all_model['MAPE_Hold_out'] = mean_absolute_percentage_error(y_test_hold, y_pred_test)
    results_all_model['R2_score_all'] = metrics.r2_score(train_y_all, y_pred_all)
    results_all_model['MAPE_all'] = mean_absolute_percentage_error(train_y_all, y_pred_all)
    ...
    actual_vs_predicted = pd.concat([actual_vs_predicted, actual_vs_predicted], axis=0)
...
def metric_weights_model(pillar, brand_name, category_name, modeling_data, feat_eng_config, req_cols, refresh_config):
    attr_df1 = modeling_data.copy()
    attr_df1['date'] = pd.to_datetime(attr_df1['date'], format="%Y-%m-%d")
    attr_df1 = attr_df1.sort_values(by='date', ascending=True)
    # attr_df1.rename(columns={'brand': 'brand_rt', 'category': 'category_rt', 'market_share': 'market_share_total_sales_rt'}, inplace=True)
    ...
    results_all_model_RF = pd.DataFrame()
    results_all_model_XGB = pd.DataFrame()
    brand_prc_cls_df = attr_df.loc[(attr_df['brand']==brand_name) & (attr_df['c_category']==category_name)]
    brand_req_df=brand_prc_cls_df.copy()
    ...
    #actual_df = brand_req_df.copy()
    brand_req_df1 = brand_req_df[idv_sel_cols_list+[feat_eng_config["weights_model"]["DV"]]+[feat_eng_config["weights_model"]["Temp"]]]
    brand_req_df1
    filtered_df=pd.DataFrame()
    null_columns =[]
    filtered_df = brand_req_df1[brand_req_df1['new_brand'] == 'Stacked Brand']
    ...
    final_idvs = list(common_elements)
    final_idvs
    brand_req_df_2 = brand_req_df[final_idvs+[feat_eng_config["weights_model"]["DV"]]+[feat_eng_config["weights_model"]["Temp"]]]
    brand_req_df_2
    brand_req_df = brand_req_df_2[brand_req_df_2['new_brand'] == brand_name]
    brand_req_df = brand_req_df[final_idvs+[feat_eng_config["weights_model"]["DV"]]]
    brand_req_df
    ...
    if len(brand_req_df) >20:
        ...
        print("len(set(brand_req_df.columns) - {feat_eng_config['weights_model']['DV']})",len(set(brand_req_df.columns) -{feat_eng_config["weights_model"]["DV"]}))

        if len(set(brand_req_df.columns) - {feat_eng_config["weights_model"]["DV"]}) > 0:
            print(len(set(brand_req_df.columns) - {feat_eng_config["weights_model"]["DV"]}))
            ...
            if feat_eng_config["weights_model"]["is_lag_considered"] == True:
                chosen_idvs = choose_best_lag_feature(brand_req_df, idv_sel_cols, feat_eng_config["weights_model"]["DV"])
                for col in list(columns_log_df.loc[pd.isnull(columns_log_df['Reasons_to_drop'])==True,'Column_names']):
                    if col not in list(chosen_idvs):
                        columns_log_df.loc[columns_log_df['Column_names']==col,'Reasons_to_drop'] = "Dropped for best lag"
                ...
                chosen_idvs = list(brand_req_df.columns)
                chosen_idvs.remove(feat_eng_config["weights_model"]["DV"]) ## if only ACV is needed and drop price in the idv list
            print("chosen_idvs : ",list(chosen_idvs))
            ...
            if len(cols_to_select) >= 2:
                for i in [2]:
                    # i = 2
                    print(i)
                    if feat_eng_config["weights_model"]["Time_series_split"] == True:
                        train_x = idvs[cols_to_select].iloc[:42,:]
                        train_y = dv[:42]
                        test_x = idvs[cols_to_select].iloc[42:,:]
                        test_y = dv[42:]
                    if feat_eng_config["weights_model"]["Random_seed_split"] == True:
                        train_x, test_x, train_y, test_y = train_test_split(idvs[cols_to_select],dv,test_size=6, random_state=i, shuffle=True)
                    train_x_all =  idvs[cols_to_select]
                    train_y_all = dv
                    ...
                    if feat_eng_config["weights_model"]["P_N_check"] == True:
                        ...
                        idv_sel_P_N_Match = idv_sel_P_N[~((idv_sel_P_N['Correlation'] < -feat_eng_config["weights_model"]["Counter_Intuitive_cut_off"]) & (idv_sel_P_N['Intutive']=='P')) |((idv_sel_P_N['Correlation'] > feat_eng_config["weights_model"]["Counter_Intuitive_cut_off"]) & (idv_sel_P_N['Intutive']=='N'))].reset_index(drop=True)
                    ...
                    if feat_eng_config["weights_model"]["Corr_file_generation"] == True :
                        if feat_eng_config["weights_model"]["P_N_check"] == True:
                            train_x_corr =  idvs[Count_intuitive_cols_filtered]
                        else:
                            train_x_corr =  idvs[cols_to_select]
                    ...
                    if feat_eng_config["weights_model"]["PCA_Transform"] == True:
                        pca = PCA(n_components = 0.95)
                        PCA_train = pca.fit_transform(train_x)
                        PCA_comp = []
                        for i in range(0,pca.n_components_,1):
...
def modelling(input_config,output_config,mapping_config,refresh_config, feat_eng_config,filter_config,storage_options, refresh_type):
    #input
    eq_sub_scale_merged_brand = pd.read_csv(output_config["data_prep"]["eq_sub_scale"], storage_options = storage_options)
    modeling_data = pd.read_csv(output_config["data_prep"]["modeling_data"], storage_options = storage_options)
    eq_sub_scale_merged_brand["date"] = pd.to_datetime(eq_sub_scale_merged_brand["date"])
    modeling_data["date"] = pd.to_datetime(modeling_data["date"])
    eq_sub_scale_merged_brand.rename(columns={"brand":"Brand","category":"Category","new_brand":"New_Brand"}, inplace=True)
    req_cols = pd.read_csv(mapping_config["idv_list"], storage_options=storage_options)
    brand_list = pd.read_csv(mapping_config["brand_list"], storage_options=storage_options)
    brand_list_dict = brand_list.groupby('brand_group_expanded')['category'].apply(list).to_dict()
    #cfa (need to bring loops outside)
    fit_summary_all_cat_py, fit_summary_all_brands_py = cfa(output_config, refresh_config, feat_eng_config, eq_sub_scale_merged_brand, req_cols, storage_options, refresh_type)

    fit_summary_all_cat_py.to_csv(output_config["cfa"]["model_results_all_category"], index=False, storage_options = storage_options)
    fit_summary_all_brands_py.to_csv(output_config["cfa"]["model_results_by_category"],index=False, storage_options = storage_options)
    #rf1
    all_pillar_results = pd.DataFrame()
