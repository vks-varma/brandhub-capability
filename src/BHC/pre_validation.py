...
def nielsen_rms_data_extraction(
    dbs_sql_hostname,
    dbs_sql_http_path,
    dbs_sql_token,
    market_description="Total US Pet Retail Plus",
    time_granularity=refresh_config["time_granularity"],
):

    conn = sql.connect(
        server_hostname=dbs_sql_hostname,
        http_path=dbs_sql_http_path,
        access_token=dbs_sql_token,
    )
    cursor = conn.cursor()
    ...
    rms_facts["RMS_ACV_Selling_wt"] = (
        rms_facts["RMS_ACV_Selling_num"] / rms_facts["RMS_ACV_Selling_den"]
    )
    ...
    aggregated_bb_rms = blue_buffalo_rms.groupby(
        ["vendor", "category", "date"], as_index=False
    ).agg(
        {
            "s_dollars": "sum",
            "s_pounds": "sum",
    ...
    non_blue_buffalo_rms = rms_facts[
        rms_facts["brand_group_expanded"] != "BLUE BUFFALO"
    ]

    # Step 5: Combine the aggregated data with the rest of the data
    rms_facts = pd.concat(
        [non_blue_buffalo_rms, aggregated_bb_rms], ignore_index=True
    )
    ...
    if time_granularity == "monthly":
        ...
        rms_facts_renamed["daysinmonth"] = rms_facts_renamed.apply(
            lambda row: days_in_month(int(row["year"]), int(row["month"])),
            axis=1,
        )

        # Convert monthly values to weekly values by multiplying by (daysinmonth / 7)
        rms_facts_renamed["total_sales"] = rms_facts_renamed["a_dollars"] * (
            rms_facts_renamed["daysinmonth"] / 7
        )
        rms_facts_renamed["s_base_dollars"] = rms_facts_renamed[
            "a_base_dollars"
        ] * (rms_facts_renamed["daysinmonth"] / 7)
        rms_facts_renamed["s_base_pounds"] = rms_facts_renamed[
            "a_base_pounds"
        ] * (rms_facts_renamed["daysinmonth"] / 7)
        rms_facts_renamed["eq_volume"] = rms_facts_renamed["a_pounds"] * (
            rms_facts_renamed["daysinmonth"] / 7
        )
        rms_facts_renamed["s_base_units"] = rms_facts_renamed[
            "a_base_units"
        ] * (rms_facts_renamed["daysinmonth"] / 7)
        rms_facts_renamed["total_units"] = rms_facts_renamed["a_units"] * (
            rms_facts_renamed["daysinmonth"] / 7
        )
        rms_facts_renamed["date"] = pd.to_datetime(
            rms_facts_renamed[["year", "month"]].assign(day=1)
        )
        rms_facts_renamed.drop(
            columns=["month", "year", "daysinmonth"], inplace=True
        )
        col_to_move = rms_facts_renamed.pop(
            "date"
        )  # Remove the column from its original position
        rms_facts_renamed.insert(
            3, "date", col_to_move
    ...
    rms_facts_renamed = rms_facts_renamed.round(
        {
            "total_sales": 0,
            "a_base_dollars": 0,
            "eq_volume": 0,
    ...
    # Sorting the results
    rms_facts_renamed = rms_facts_renamed.sort_values(
        by=["vendor", "brand_group_expanded", "category", "date"]
    )

    rms_facts_renamed["average_price"] = (
        rms_facts_renamed["total_sales"] / rms_facts_renamed["total_units"]
    )

    # First, group by 'date' and 'CATEGORY_RT' to compute the sum of 'total_sales' for each group
    rms_facts_renamed["Category_total_sales"] = rms_facts_renamed.groupby(
        ["date", "category"]
    )["total_sales"].transform("sum")

    # Then, compute the market share by dividing 'total_sales' by 'Category_total_sales'
    rms_facts_renamed["Market_share_total_sales"] = (
        rms_facts_renamed["total_sales"]
        / rms_facts_renamed["Category_total_sales"]
    )
...
def create_percentage_columns(df):
    ...
    rating_count_columns = [
        ...
        if col.startswith("ratings_") and col.endswith("_count")
    ]
    star_rating_pattern = r"^ratings_reviews_[1-5]_star_ratings$"
    star_rating_columns = [
        col for col in df.columns if re.match(star_rating_pattern, col)
    ]
...
def create_inverse_metrics(df, inverse_logic_df):
    ...
    for _, row in inverse_logic_df.iterrows():
        ...
        if operation == "sub":
            ...
            df_copy[new_col_name] = np.where(
                df_copy[raw_col].isnull(), np.nan, value - df_copy[raw_col]
            )
        ...
        # Handle addition operation
            ...
            df_copy[new_col_name] = df_copy[cols_to_add].apply(
                lambda x: x.sum() if not x.isnull().all() else np.nan, axis=1
            )
...
def create_share_metrics(df):
    ...
    for (date_val, category_name), cat_df in df.groupby(["date", "category"]):
        ...
        category_sum_review_count = cat_df[
            "ratings_reviews_review_count"
        ].sum()
        ...
        for brand_name, br_cat_df in cat_df.groupby("brand_group_expanded"):
            ...
            br_cat_df = br_cat_df.assign(
                search_searchvolume_share=br_cat_df["search_searchvolume"]
                / category_sum_search_volume,
                ratings_reviews_review_count_share=br_cat_df[
                    "ratings_reviews_review_count"
                ]
                / category_sum_review_count,
            )
...
def load_and_preprocess_data(
    file_path, date_cols=None, drop_cols=None, sales_or_harmonized=None
):
    ...
    if (
        sales_or_harmonized == "sales"
        and "Year_RT" in df.columns
        and "Month_RT" in df.columns
    ):
        df["date"] = pd.to_datetime(
            df["Year_RT"].astype(str)
            + "-"
            + df["Month_RT"].astype(str).str.zfill(2)
            + "-01"
        )
...
def rename_trends_category(df):
    ...
    df.loc[
        df["brand_group_expanded"].isin(dog_treats_brands)
        & (df["category"] == "DOG FOOD"),
        "category",
    ] = "DOG TREATS ONLY"
    df.loc[
        df["brand_group_expanded"].isin(cat_treats_brands)
        & (df["category"] == "CAT FOOD"),
        "category",
    ] = "CAT TREATS ONLY"
...
def add_idv_and_brand_lists(df_grouped, data_sufficiency_check=False):
    ...
    idv_list = pd.read_csv(
        mapping_config["idv_list"], storage_options=storage_options
    )

    idv_list = idv_list[idv_list["Select"] == "Y"].drop(
        [
            "Unnamed: 0",
            "metric_name",
            "Vendor Metric Group",
    ...
    df_grouped = pd.merge(
        df_grouped,
        idv_list,
        left_on=["category", "variable"],
        right_on=["product_category_idv", "idv_for_model_corrected"],
        how="left",
    )
    ...
    df_grouped["data_source"] = (
        ...
        .str.split("_")
        .str[0]
        .replace({"neilsen": "neilsen_panel", "ratings": "ratings_reviews"})
    )
    ...
    rms_metrics = [
        ...
        "total_units_rt",
        "average_price_rt",
        "market_share_total_sales_rt",
    ]

    df_grouped.loc[df_grouped["variable"].isin(rms_metrics), "data_source"] = (
        "RMS"
    )
    ...
    df_grouped["category"] = np.where(
        (df_grouped["brand_group_expanded"].isin(dog_treats_brands))
        & (df_grouped["category"] == "DOG FOOD"),
        "DOG TREATS ONLY",
        np.where(
            (df_grouped["brand_group_expanded"].isin(cat_treats_brands))
            & (df_grouped["category"] == "CAT FOOD"),
            "CAT TREATS ONLY",
            df_grouped["category"],
        ),
    )
    ...
    brand_list = brand_list.rename(
        columns={"brand_group_expanded": "brand_bl", "category": "category_bl"}
    )
    df_grouped_bl = pd.merge(
        df_grouped,
        brand_list,
        left_on=["brand_group_expanded", "category"],
        right_on=["brand_bl", "category_bl"],
        how="left",
    )

    df_grouped_bl["brand_list"] = np.where(
        df_grouped_bl["brand_bl"].notna(), "Y", "N"
    )
    df_grouped_bl["idv_list"] = df_grouped_bl["Select"].fillna("N")
    if data_sufficiency_check == False:
        df_grouped_bl = df_grouped_bl.drop(
            ["Select", "brand_bl", "category_bl"], axis=1
        )
...
def harmonized_data_comparison(
    old_data_path,
    new_data_path,
    sales_or_harmonized,
    ...
    if sales_or_harmonized == "harmonized":
        ...
        old_hd = load_and_preprocess_data(
            old_data_path,
            date_cols=["date"],
            drop_cols=drop_cols,
            sales_or_harmonized="harmonized",
        )
        new_hd = load_and_preprocess_data(
            new_data_path,
            date_cols=["date"],
            drop_cols=drop_cols,
            sales_or_harmonized="harmonized",
        ...
        panel_rename_df = pd.read_csv(
            mapping_config["panel_rename_mapping"],
        ...
        old_hd = old_hd[
            (old_hd["date"] >= start_date) & (old_hd["date"] <= end_date)
        ]
        new_hd = new_hd[
            (new_hd["date"] >= start_date) & (new_hd["date"] <= end_date)
        ]
        ...
        value_vars = [
            ...
            not in [
                ...
                "neilsen_panel_volume_lbs",
                "neilsen_panel_voume_lbs",
            ]
        ]
        value_vars1 = [
            col
            for col in old_hd.columns
            if col
            not in [
                "Unnamed: 0.1",
                "brand_group_expanded",
        ...
        new_hd_lf = new_hd.melt(
            id_vars=["brand_group_expanded", "category", "date"],
            value_vars=value_vars,
            var_name="variable",
            value_name="value_new",
        )
        old_hd_lf = old_hd.melt(
            id_vars=["brand_group_expanded", "category", "date"],
            value_vars=value_vars1,
            var_name="variable",
            value_name="value_old",
        )
        print("new_hd_lf head:", new_hd_lf.head())
        print("old_hd_lf head:", old_hd_lf.head())
        new_hd_lf["value_new"] = pd.to_numeric(
            new_hd_lf["value_new"], errors="coerce"
        )
        old_hd_lf["value_old"] = pd.to_numeric(
            old_hd_lf["value_old"], errors="coerce"
        )
        ...
        df_joined["difference%"] = (
            (df_joined["value_new"] - df_joined["value_old"])
            / df_joined["value_old"]
        ) * 100
        ...
        df_grouped = df_grouped.rename(
            columns={"difference%": "avg_difference%"}
        )
        ...
        old_sales = load_and_preprocess_data(
            old_data_path,
            drop_cols=["Unnamed: 0"],
            sales_or_harmonized="sales",
        )
        new_sales = load_and_preprocess_data(
            new_data_path,
            drop_cols=["Unnamed: 0"],
            sales_or_harmonized="sales",
        )
        ...
        old_sales_lf = pd.melt(
            old_sales,
            id_vars=["BRAND_RT", "CATEGORY_RT", "date"],
            value_vars=value_vars,
            var_name="variable",
            value_name="value_old",
        )
        new_sales_lf = pd.melt(
            new_sales,
            id_vars=["BRAND_RT", "CATEGORY_RT", "date"],
            value_vars=value_vars,
            var_name="variable",
        ...
        sales_merged = pd.merge(
            old_sales_lf,
            new_sales_lf,
            on=["BRAND_RT", "CATEGORY_RT", "date", "variable"],
        )
        ...
        if (
            "value_new" in sales_merged.columns
            and "value_old" in sales_merged.columns
        ):
            sales_merged["difference%"] = (
                (sales_merged["value_new"] - sales_merged["value_old"])
                / sales_merged["value_old"]
                * 100
            )
        else:
            raise KeyError(
                "Expected columns 'value_new' or 'value_old' are not present in the merged DataFrame"
            )

        # Group by and calculate average percentage difference
        sales_grouped = (
            sales_merged.groupby(["BRAND_RT", "CATEGORY_RT", "variable"])
            .agg({"difference%": "mean"})
            .reset_index()
        )
...
def generate_filtered_df(
    data, sales_or_harmonized, output_path, storage_options
):

    if sales_or_harmonized == "harmonized":
        # ata['brand'] = data['brand_group_expanded'] + " " + data['category']
        filtered_data = data[
            (data["idv_list"] == "Y") & (data["brand_list"] == "Y")
        ]
        filtered_data = filtered_data.rename(
            columns={"brand_group_expanded": "brand"}
        )
        print(filtered_data.columns)
        filtered_data["brand"] = (
            filtered_data["brand"]
            + " "
            + filtered_data["category"].str.split().str[0]
        )
        deviation_df = filtered_data[
            (filtered_data["avg_difference%"] > 5)
            | (filtered_data["avg_difference%"] < -5)
        ].copy()
        ...
        deviation_df["avg_difference%"] = (
            deviation_df["avg_difference%"].round(1).astype(str) + "%"
        )

    elif sales_or_harmonized == "sales":
        data["brand"] = data["BRAND_RT"] + " " + data["CATEGORY_RT"]
        filtered_data = data[(data["avg_difference%"] > 5)][
            ["brand", "variable", "avg_difference%"]
        ]
        filtered_data["brand"] = filtered_data["brand"].replace(
            {" TREATS ONLY": "", " LITTER": "", " FOOD": ""}, regex=True
        )
    ...
    success_percentage = round(
        (brands_without_deviation / total_brands) * 100, 2
    )
...
def harmonized_data_sufficiency(
    data_path, start_date="2019-08-01", end_date="2024-06-01"
):
    """
    Compares two datasets and returns a comparison report with difference percentages.
    ...
    data_availability_hd = (
        hd_idv_bl_lf.groupby(
            [
                "brand_group_expanded",
                "category",
                "variable",
        ...
        .agg(
            non_null_count=pd.NamedAgg(
                column="value", aggfunc=lambda x: x.notnull().sum()
            ),  # Count of non-null data points
            total_count=pd.NamedAgg(
                column="value", aggfunc="count"
            ),  # Total count of data points
            availability_percentage=pd.NamedAgg(
                column="value", aggfunc=lambda x: x.notnull().mean() * 100
            ),  # Percentage of non-null values
            start_date=pd.NamedAgg(
                column="date", aggfunc=lambda x: x.loc[x.notnull()].min()
            ),  # Min date where value is not null
            end_date=pd.NamedAgg(
                column="date", aggfunc=lambda x: x.loc[x.notnull()].max()
            ),  # Max date where value is not null
        )
        .reset_index()
    )
...
def harmonized_data_extraction(
    time_granularity=refresh_config["time_granularity"],
):
    if time_granularity == "weekly":
        harmonized_df = spark.sql(
            f"select * from brand_hub_gold.harmonized_data_weekly_view"
        )
    elif time_granularity == "monthly":
        harmonized_df = spark.sql(
            f"select * from brand_hub_gold.harmonized_data_view"
        )
    harmonized_df_corrected_metric_name = harmonized_df.withColumn(
        "metric_name_new",
        concat_ws(
            "_", "data_source", "metric_type", "metric_name", "user_segment"
        ),
    )
    harmonized_df_corrected_metric_name = (
        harmonized_df_corrected_metric_name.withColumn(
            "metric_name_new", lower("metric_name_new")
        ).withColumn("metric_name_new", translate("metric_name_new", " ", "_"))
    )

    harmonized_df_corrected_metric_name = (
        harmonized_df_corrected_metric_name.groupBy(
            "brand_group_expanded", "category", "date"
        )
        .pivot("metric_name_new")
        .avg("metric_value")
    )

    harmonized_df_corrected_metric_name = (
        harmonized_df_corrected_metric_name.orderBy(
            ["brand_group_expanded", "category", "date"]
        )
    )
    harmonized_df_corrected_metric_name = (
        harmonized_df_corrected_metric_name.toPandas()
    )
...
def harmonized_data_prep(
    ...
    col_name_rename_df = pd.read_csv(
        mapping_config["metrics_rename_mapping"],
        storage_options=storage_options,
    )
    inverse_logic_df = pd.read_csv(
        mapping_config["inverse_logic_mapping"],
        storage_options=storage_options,
    )
    ...
    if platform_type == "databricks":
        if spark.conf.get(
            ...
            nielsen_rms_data = nielsen_rms_data_extraction(
                dbs_sql_hostname, dbs_sql_http_path, dbs_sql_token
            )
        else:
            # nielsen_rms_data = pd.read_csv(input_config["current_sales_data"], storage_options=storage_options)
            nielsen_rms_data = pd.read_csv(
                "abfss://restricted-dataoperations@npusdvdatalakesta.dfs.core.windows.net/staging/cmi_brand_hub/fazil/rms_tenten_monthly_19_9_24_blue.csv",
                storage_options=storage_options,
            )
            nielsen_rms_data.columns = nielsen_rms_data.columns.str.lower()
            # Ensure the correct column names are used
            nielsen_rms_data["date"] = pd.to_datetime(
                nielsen_rms_data[["year_rt", "month_rt"]]
                .rename(columns={"year_rt": "year", "month_rt": "month"})
                .assign(day=1)
            )
            ...
            nielsen_rms_data.rename(
                columns={
                    "eq_volume": "equalized_volume",
                    "brand_rt": "brand_group_expanded",
                    "category_rt": "category",
                },
                inplace=True,
            )
            nielsen_rms_data["average_price"] = (
                nielsen_rms_data["total_sales"]
                / nielsen_rms_data["total_units"]
            )
    ...
    #     harmonized_df = pd.read_csv(input_config["harmonized_data"])
        ...
        nielsen_rms_data = pd.read_csv(
            input_config["current_sales_data"], storage_options=storage_options
        )
        nielsen_rms_data.columns = nielsen_rms_data.columns.str.lower()
        # Ensure the correct column names are used
        nielsen_rms_data["date"] = pd.to_datetime(
        ...
        nielsen_rms_data["average_price"] = (
            nielsen_rms_data["total_sales"] / nielsen_rms_data["total_units"]
        )

    harmonized_df = pd.read_csv(
        input_config["current_harmonized_data"],
        storage_options=storage_options,
    )
    ...
    harmonized_df.to_csv(
        f"{output_config['raw_input_data']}",
        index=False,
        storage_options=storage_options,
    )
    ...
    col_name_rename_df = col_name_rename_df.dropna(
        subset="DataHub Name", how="any"
    )

    rms_harmonized_data = pd.merge(
        harmonized_df,
        nielsen_rms_data,
        on=["brand_group_expanded", "category", "date"],
        how="left",
    )
    ...
    print(
        "columns_presents:", "ratings_reviews_review_" in df.columns.tolist()
    )
    df = df.rename(columns=dict(col_name_rename_df.values))
    print(
        "columns_presents:",
        "ratings_reviews_review_count" in df.columns.tolist(),
    )
    ...
    processed_harmonized_data.to_csv(
        f"{output_config['processed_input_data']}",
        index=False,
        storage_options=storage_options,
    )

    nielsen_rms_data.to_csv(
        f"{output_config['processed_sales_data']}",
        index=False,
        storage_options=storage_options,
    )
...
def data_preformatting(
    ...
    processed_harmonized_data = harmonized_data_prep(
        input_config,
        output_config,
        mapping_config,
        refresh_config,
