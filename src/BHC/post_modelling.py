...
def calculate_category_sums(equity_data, dv_column):
    ...
    """
    equity_data_agg = (
        equity_data.groupby(["category", "brand_group_expanded"])[dv_column]
        .mean()
        .reset_index()
    )
    if dv_column == "market_share_total_sales":
        equity_data_agg.rename(
            columns={dv_column: "Market share - mean"}, inplace=True
        )
        category_sums = (
            equity_data_agg.groupby("category")["Market share - mean"]
            .sum()
            .reset_index()
        )
        category_sums.rename(
            columns={"Market share - mean": "category_Market share - sum"},
            inplace=True,
        )
    elif dv_column == "equalized_volume":
        equity_data_agg.rename(
            columns={dv_column: "Equalized Volume - mean"}, inplace=True
        )
        category_sums = (
            equity_data_agg.groupby("category")["Equalized Volume - mean"]
            .sum()
            .reset_index()
        )
        category_sums.rename(
            columns={
                "Equalized Volume - mean": "category_Equalized Volume - sum"
            },
            inplace=True,
        )
    elif dv_column == "total_sales":
        equity_data_agg.rename(
            columns={dv_column: "Total Sales - mean"}, inplace=True
        )
        category_sums = (
            equity_data_agg.groupby("category")["Total Sales - mean"]
            .sum()
            .reset_index()
        )
        category_sums.rename(
            columns={"Total Sales - mean": "category_Total Sales - sum"},
            inplace=True,
        )
...
def dv_weighted_shap(
    df,
    model_results_sub,
    end_date,
    categories_list,
    feat_eng_config,
    DV_previous_years_to_take=1,
):
    ...
    df_sub = df_sub[
        (df_sub["date"] >= start_date) & (df_sub["date"] <= end_date)
    ]
    ...
    equity_data_agg = equity_data_agg.merge(
        category_sums, on="category", how="left"
    )

    # Prepare model results DataFrame for merging
    model_results_sub.columns = model_results_sub.columns.str.lower()
    model_results_sub.rename(
        columns={"category": "category", "brand_group_expanded": "brands"},
        inplace=True,
    )
    merged_results = model_results_sub.merge(
        equity_data_agg,
        left_on=["brands", "category"],
        right_on=["brand_group_expanded", "category"],
        how="left",
    )
    ...
    shap_column_map = {
        "market_share_total_sales": (
            "Market share - mean",
            "category_Market share - sum",
        ),
        "equalized_volume": (
            "Equalized Volume - mean",
            "category_Equalized Volume - sum",
        ),
    ...
    if mean_column and sum_column:
        merged_results["weighted_shap"] = (
            merged_results["shap values"] * merged_results[mean_column]
        ) / merged_results[sum_column]

    # Aggregate results at category and overall levels
    category_agg = (
        merged_results.groupby(["category", "pillar", "var"])["weighted_shap"]
        .mean()
        .reset_index()
    )
    category_agg.rename(
        columns={"weighted_shap": "Shap Value - mean"}, inplace=True
    )

    overall_agg = (
        category_agg.groupby(["pillar", "var"])["Shap Value - mean"]
        .mean()
        .reset_index()
    )
...
def merge_and_filter(model_df, cfa_df, pillars, by_category=False):
    if by_category:
        merged_df = pd.merge(
            model_df, cfa_df, on=["var", "pillar", "category"], how="outer"
        )
    else:
        merged_df = pd.merge(
            model_df, cfa_df, on=["var", "pillar"], how="outer"
        )
...
def calculate_weights(df, shap_col="shap value - mean", cfa_col="est.std"):
    """Calculating Weight from cfa and rf results

    Args:
        df (DataFrame): Data with both cfa and rf results
        shap_col (str, optional): shape value column. Defaults to "shap value - mean".
        cfa_col (str, optional): cfa estimate column. Defaults to "est.std".

    Returns:
        DataFrame: Data with the weighted score for each metric against it's corresponding
    """
    df["abs_shap"] = df[shap_col].abs()
    df["abs_cfa_est"] = df[cfa_col].abs()

    sum_shap = (
        df.groupby("pillar")["abs_shap"]
        .sum()
        .rename("metrics_sum_shap")
        .reset_index()
    )
    sum_cfa = (
        df.groupby("pillar")["abs_cfa_est"]
        .sum()
        .rename("metrics_sum_cfa")
        .reset_index()
    )

    df = df.merge(sum_shap, on="pillar", how="left").merge(
        sum_cfa, on="pillar", how="left"
    )
...
def apply_equal_weights(df, pillar_name, config_key, filter_config):
...
def prepare_weights(
    all_cat_pillars,
    by_cat_pillars,
    cfa_results_all_cat_sub,
    cfa_results_by_cat_sub,
    model_results_pillar_all_cat,
    model_results_sub_agg,
    filter_config,
):
    ...
    all_cat_weights = merge_and_filter(
        model_results_pillar_all_cat, cfa_results_all_cat_sub, all_cat_pillars
    )
    by_cat_weights_merged = merge_and_filter(
        model_results_sub_agg,
        cfa_results_by_cat_sub,
        by_cat_pillars,
        by_category=True,
    ...
    by_cat_weights = pd.concat(
        [
            calculate_weights(
                by_cat_weights_merged[
                    by_cat_weights_merged["category"] == category
                ]
            )
    ...
    apply_equal_weights(
        all_cat_weights,
        "awareness_pillar",
        "give_awareness_metrics_equal_weightage",
        filter_config,
    )
    apply_equal_weights(
        all_cat_weights,
        "advocacy_pillar",
        "give_advocacy_metrics_equal_weightage",
        filter_config,
    )
    ...
    all_cat_weights_expanded = pd.concat(
        [
            all_cat_weights.assign(category=category)
            for category in cfa_results_by_cat_sub["category"].unique()
        ],
        ignore_index=True,
    )
    ...
    all_weights = pd.concat(
        [by_cat_weights, all_cat_weights_expanded], ignore_index=True
    ).rename(columns={"var": "metric"})
...
def prepare_cfa_subset(cfa_data, exclude_pillars):
    ...
    ][["lhs", "rhs", "est.std", "Brands", "Category"]].rename(
        columns={"lhs": "Pillar", "rhs": "Var"}
    )
...
def weights_creation(
    fit_summary_all_cat,
    fit_summary_all_brands,
    refresh_config,
    feat_eng_config,
    ...
    cfa_results_all_cat_sub = prepare_cfa_subset(
        fit_summary_all_cat, exclude_pillars
    )
    cfa_results_by_cat_sub = prepare_cfa_subset(
        fit_summary_all_brands, exclude_pillars
    )

    model_results_sub = model_results[
        ["Shap Features", "shap values", "Brand", "Category", "pillar"]
    ].drop_duplicates()
    model_results_sub.rename(
        columns={
            "Brand": "brands",
    ...
    if feat_eng_config["cfa"]["perform_weighted_average_DV"]:
        model_results_sub_agg, model_results_pillar_all_cat = dv_weighted_shap(
            equity_dt,
            model_results_sub,
            refresh_config["end_date"],
            category_list_pillar,
            feat_eng_config,
        )
    else:
        model_results_sub_agg = model_results_sub.groupby(
            ["category", "pillar", "var"], as_index=False
        )["shap values"].mean()
        model_results_sub_agg.rename(
            columns={"shap values": "Shap Value - mean"}, inplace=True
        )
        model_results_pillar_all_cat = model_results_sub_agg.groupby(
            ["pillar", "var"], as_index=False
        )["Shap Value - mean"].mean()
...
def create_pillar_scores(
    ...
    scaled_df_sub["date"] = pd.to_datetime(scaled_df_sub["date"])
    equity_dt["date"] = pd.to_datetime(equity_dt["date"])
    ...
    metric_weights = weights_sheet[
        (weights_sheet["pillar"] == pillar)
        & (weights_sheet["category"] == category)
    ]
    ...
    metric_weights = metric_weights[
        metric_weights["metric"].isin(relevant_cols)
    ].sort_values(by="metric")

    # Calculate weighted scores for the pillar
    scaled_df_sub[pillar] = scaled_df_sub[relevant_cols].dot(
        metric_weights["weight"].values
    )

    # Append the results to eq_sub2
    # eq_sub2 = pd.concat([eq_sub2, scaled_df_sub], ignore_index=True)
    eq_sub2 = scaled_df_sub
    # Drop unnecessary columns if present
    eq_sub2.drop(columns=["new_brand", "v1"], inplace=True, errors="ignore")

    # Transform to long format
    index_df_long = pd.melt(
        eq_sub2, id_vars=["date", "brand", "category"], var_name="var"
    )

    # Prepare mapping for merging pillars
    mapping_subset = req_cols[
        ["idv_for_model_corrected", "Equity Pillar", "product_category_idv"]
    ].rename(
        columns={
            "idv_for_model_corrected": "var",
            "product_category_idv": "category",
        }
    )
    mapping_subset["var"] = mapping_subset["var"].str.lower()

    # Merge pillar mappings
    index_df_long = pd.merge(
        index_df_long, mapping_subset, on=["var", "category"], how="left"
    )
    ...
    for df in [eq_sub2, index_df_long]:
        df["date"] = pd.to_datetime(df["date"], utc=False)

    # Merge DV data with the pillar scores data
    # if "market_share" not in eq_sub2.columns or "equalized_volume" not in eq_sub2.columns or feat_eng_config["weights_model"]["DV"] not in eq_sub2.columns:
    # eq_sub2 = pd.merge(eq_sub2, dv_data, on=['date', 'brand', 'category'], how='left')
    # index_df_long = pd.merge(index_df_long, dv_data, on=['date', 'brand', 'category'], how='left')

    return eq_sub2, index_df_long
...
def pillar_trend_creation(br_cat_df, brand, category, pillar, refresh_config):
    ...
    pillar_cols = [col for col in br_cat_df.columns if "_pillar" in col]
    # print('pillar_cols:', pillar_cols)

    # Keep only relevant columns
    br_cat_df = br_cat_df[
        ["date", "brand", "category"] + pillar_cols + [refresh_config["dv"]]
    ]
...
def lag_addition(dataset, refresh_config):
        ...
        for category in brand_data["category"].unique():
            ...
            for col in basecols:
                for lag in range(1, 4):  # Create lags 1 to 3
                    category_data[f"{col}_lag_{lag}"] = category_data[
                        col
                    ].shift(lag)
...
def run_model_analysis(
    ...
    if model_type == "Brute_force":
        params = {
            "alpha": feat_eng_config["importance_model"]["hyperparameters"][
                "Brute_force"
            ]["grid_search"]["alpha"],
            "random_state": feat_eng_config["importance_model"][
                "hyperparameters"
            ]["Brute_force"]["grid_search"]["random_state"],
        }
        model = Ridge(
            positive=feat_eng_config["importance_model"]["hyperparameters"][
                "Brute_force"
            ]["positive"],
            random_state=feat_eng_config["importance_model"][
                "hyperparameters"
            ]["Brute_force"]["random_state"],
        )
        ...
        params = {
            "max_depth": feat_eng_config["importance_model"][
                "hyperparameters"
            ]["RandomForest"]["grid_search"]["max_depth"],
            "n_estimators": feat_eng_config["importance_model"][
                "hyperparameters"
            ]["RandomForest"]["grid_search"]["n_estimators"],
            "max_features": feat_eng_config["importance_model"][
                "hyperparameters"
        ...
        params = {
            "max_depth": feat_eng_config["importance_model"][
                "hyperparameters"
            ]["XGBoost"]["grid_search"]["max_depth"],
            "n_estimators": feat_eng_config["importance_model"][
                "hyperparameters"
            ]["XGBoost"]["grid_search"]["n_estimators"],
            "learning_rate": feat_eng_config["importance_model"][
                "hyperparameters"
            ]["XGBoost"]["grid_search"]["learning_rate"],
            "random_state": feat_eng_config["importance_model"][
                "hyperparameters"
        ...
        feat_importance = pd.DataFrame(
            {
                ...
                "Feature Importance/coefficient": params,
            }
        )
    elif model_type == "RandomForest":
        feat_importance = pd.DataFrame(
            list(zip(X_train.columns, model.feature_importances_)),
            columns=["Features", "Feature Importance/coefficient"],
        ).sort_values(by="Feature Importance/coefficient", ascending=False)
    elif model_type == "XGBoost":
        feat_importance = pd.DataFrame(
...
def pillar_importance_model(
    ...
    results_all_model = pd.DataFrame()
    final_df = lag_addition(brand_df, refresh_config)
    final_df.rename(columns={"category": "category_new"}, inplace=True)

    final_df.loc[final_df.category_new == "CAT TREATS ONLY", "category"] = (
        "CAT FOOD"
    )
    final_df.loc[final_df.category_new == "DOG TREATS ONLY", "category"] = (
        "DOG FOOD"
    )
    ...
    final_df.loc[final_df.category_new == "CAT LITTER", "category"] = (
        "CAT LITTER"
    )
    ...
    rms_teneten_df.rename(
        columns={"brand_group_expanded": "brand"}, inplace=True
    )

    # final_df.drop(columns=['equalized_volume'],inplace=True)

    final_df = final_df.merge(
        rms_teneten_df, on=["date", "category", "brand"], how="left"
    )

    # print(list(final_df.columns))

    past_future_trend_cols = [
        cols for cols in final_df.columns if "_trend_past_future" in cols
    ]
    past_trend_cols = [
        cols
        for cols in final_df.columns
        if "_trend_past" in cols and cols not in past_future_trend_cols
    ]
    raw_cols = [
        cols
        for cols in final_df.columns
        if cols not in past_trend_cols and cols not in past_future_trend_cols
    ]
    ...
    cat_br_df = final_df[
        (final_df["brand"] == brand) & (final_df["category_new"] == category)
    ]
    null_count_df = pd.DataFrame(
        cat_br_df.isnull().sum(), columns=["null count"]
    )
    null_cols = null_count_df[
        null_count_df["null count"] > cat_br_df.shape[0] * 0.5
    ].index
    if len(null_cols) > 0:
        cat_br_df.drop(columns=null_cols, inplace=True)
    cols_exclude = ["date", "brand", "category_new"]
    cat_br_df = cat_br_df.select_dtypes(include=["number"])
    cat_br_df = cat_br_df.fillna(cat_br_df.mean())

    brand_perceptions_pillar = [
        cols
        for cols in cat_br_df.columns
        if cols.startswith("brand_perceptions")
    ]
    product_feedback_pillar = [
        cols
        for cols in cat_br_df.columns
        if cols.startswith("product_feedback")
    ]
    advocacy_pillar = [
        cols for cols in cat_br_df.columns if cols.startswith("advocacy")
    ]
    awareness_pillar = [
        cols for cols in cat_br_df.columns if cols.startswith("awareness")
    ]
    consideration_pillar = [
        cols for cols in cat_br_df.columns if cols.startswith("consideration")
    ]
    loyalty_pillar = [
        cols for cols in cat_br_df.columns if cols.startswith("loyalty")
    ]
    ...
    if feat_eng_config["importance_model"]["lags_added"] == False:
        flatten_pillars_list = [
            cols for cols in flatten_pillars_list if ("_lag") not in cols
        ]  # dropping lag metrics
    past_future_trend_cols = [
        cols for cols in flatten_pillars_list if "_trend_past_future" in cols
    ]
    past_trend_cols = [
        cols
        for cols in flatten_pillars_list
        if "_trend_past" in cols and cols not in past_future_trend_cols
    ]
    raw_cols = [
        cols
    ...
    for j in [cols_to_select]:
        ...
        modeldf = cat_br_pillar_df[
            matched_cols + [feat_eng_config["importance_model"]["DV"]]
        ]

        idvs = modeldf.drop(
            feat_eng_config["importance_model"]["DV"], axis=1
        )  # feature matrix
        ...
        if idvs.shape[1] > 2:
            if feat_eng_config["importance_model"]["standardize"]:
                ...
                if len(idvs_scaled.columns) >= 2:
                    for i in [2]:
                        # print(i)
                        if feat_eng_config["importance_model"][
                            "Time_series_split"
                        ]:
                        ...
                        if feat_eng_config["importance_model"][
                            ...
                            corr_raw_df_full = pd.concat(
                                [corr_raw_df_full, corr_pairs]
                            )

                        # if refresh_config["importance_model"]["Brute_force"][
                        #     "run"
                        # ]:
                        ...
                        if refresh_config["importance_model"]["RandomForest"][
                            "run"
                        ]:
                            results = run_model_analysis(
                        ...
                        if refresh_config["importance_model"]["XGBoost"][
                            "run"
                        ]:
                            results = run_model_analysis(
                                model_type="XGBoost",
...
def compute_scaled_scores(
    date, category, df1, grouped, pillar_columns, columns_to_mean, only_pillars
):
    """
    ...
    subset = grouped[
        (grouped["date"] == date) & (grouped["category"] == category)
    ]

    for column_name in pillar_columns if only_pillars else columns_to_mean:
        new_column_name = f"{column_name}_scores"
        average = subset[column_name].values[
            0
        ]  # Assumes there's at least one row in subset
        subset1[new_column_name] = (subset1[column_name] - average) * 100 + 100
    return subset1


# Main function to set up the data and prepare for processing
def scaled_scores_prep(
    pillar_data, feat_eng_config, filter_config, refresh_config
):
    # Convert date column to datetime
    pillar_data["date"] = pd.to_datetime(
        pillar_data["date"], format="%Y-%m-%d"
    )

    # Define relevant columns for grouping and scaling
    pillar_columns = [
        col for col in pillar_data.columns if col.endswith("_pillar")
    ]
    additional_columns = ["date", "category", refresh_config["dv"], "brand"]

    if filter_config["scaled_score"]["only_pillars"]:
        grouped = (
            pillar_data.groupby(["date", "category"])[pillar_columns]
            .mean()
            .reset_index()
        )
        df1 = pillar_data[additional_columns + pillar_columns]
        columns_to_mean = None
    else:
        columns_to_mean = pillar_data.columns.difference(
            ["date", "brand", "category", refresh_config["dv"]]
        ).tolist()
        grouped = (
            pillar_data.groupby(["date", "category"])[columns_to_mean]
            .mean()
            .reset_index()
        )
        df1 = pillar_data
...
def data_hub_data_format_code(
    ...
    attr_df1.columns = (
        attr_df1.columns.str.replace(" - ", "_")
        .str.replace(" ", "_")
        .str.lower()
    )
    ...
    merged_df = pd.merge(
        attr_df,
        df[["category", "var", "Equity Pillar", "weight"]],
        on=["category", "var", "Equity Pillar"],
        how="left",
    )
    ...
    merged_df["Category_MC_Avg"] = merged_df.groupby(
        ["date", "category", "var"]
    )["metric contribution"].transform("mean")
    merged_df["MC diff to catg avg"] = (
        merged_df["metric contribution"] - merged_df["Category_MC_Avg"]
    )

    # Identify if Equity Pillar
    merged_df["Is Pillar"] = merged_df["Equity Pillar"].str.endswith(
        "_pillar"
    ) & (merged_df["Equity Pillar"] != "")

    # Preprocess scaled scores
    filtered_df = scaled_scores_long[
        scaled_scores_long["variable"].str.endswith("_scores")
    ].copy()
    filtered_df["var"] = filtered_df["variable"].str.replace(
        "_scores$", "", regex=True
    )
    ...
    merged_df1 = pd.merge(
        merged_df,
        filtered_df[["date", "brand", "category", "var", "column_value"]],
        on=["date", "brand", "category", "var"],
        how="left",
    )
    ...
    # if refresh_config["importance_model"]["RandomForest"] == True:
    #     model_name = "RandomForest"
    # elif refresh_config["importance_model"]["XGBoost"] == True:
    #     model_name = "XGBoost"
    # elif refresh_config["importance_model"]["Brute_force"] == True:
    #     model_name = "Brute_force"

    filtered_imp_model_results = imp_model_results[
        imp_model_results["Model"] == model_name
    ][["Brand", "Category", "Shap Features", "shap values"]].copy()

    # Calculate the sum of SHAP values and relative importance
    sum_shap_values = filtered_imp_model_results.groupby(
        ["Brand", "Category"]
    )["shap values"].transform("sum")
    filtered_imp_model_results["Relative Importance"] = (
        filtered_imp_model_results["shap values"] / sum_shap_values
    )

    # Preprocess IDV list
    attr_df4 = idv_list.rename(
        columns={"idv_for_model_corrected": "var"}
    ).drop_duplicates()
    ...
    merged_df3 = pd.merge(
        selected_columns,
        attr_df4[["var", "metric_name", "data_source", "Vendor Metric Group"]],
        on=["var"],
        how="left",
    )
...
def scorecard_summary_creation(df):
    df = df.drop(
        columns=[
            "value",
            "metric contribution",
...
def prepare_harmonized_data(harmonized_data):
    ...
    df_long = pd.melt(
        harmonized_data,
        id_vars=["brand_group_expanded", "category", "date"],
        var_name="var",
        value_name="metric_raw_value",
    )
...
def merge_and_prepare_summary(
    detailed, df_long, price_class_mapping, dashboard_metric_names_mapping
):
    price_class_mapping["Price Class"] = price_class_mapping[
        "Price Class"
    ].fillna("#NA")
    detailed["date"] = pd.to_datetime(detailed["date"])

    summary_hd = pd.merge(
        detailed, df_long, on=["brand", "category", "var", "date"], how="left"
    )
    summary_hd_price = pd.merge(
        summary_hd,
        price_class_mapping,
        left_on=["brand", "category"],
        right_on=["Brand", "Category"],
        how="left",
    )
    ...
    summary_renamed = pd.merge(
        summary_hd_price,
        dashboard_metric_names_mapping,
        left_on=["var"],
        right_on=["Scorecard_attribute"],
        how="left",
    )
    summary_renamed["var"] = summary_renamed[
        "Scorecard_attribute_updated"
    ].fillna(summary_renamed["var"])
    summary_renamed = summary_renamed.drop(
        columns=["Scorecard_attribute", "Scorecard_attribute_updated"]
    )

    summary_renamed["Equity Pillar"] = (
        summary_renamed["Equity Pillar"]
        .str.replace("_pillar", "", regex=False)
        .str.replace("_", " ")
        .str.title()
    )
...
def clean_and_rename_summary(summary_df):
    ...
    summary_df = summary_df.drop(
        columns=["Brand", "Category", "year", "month"]
    )
    ...
    summary_df = summary_df[
        ~(
            summary_df["weight"].isnull()
            & (summary_df["metric_type"] == "Metric")
        )
    ]
...
def prepare_pillar_importances(pillar_importances, price_class_mapping):
    updated_pillars = pd.merge(
        pillar_importances,
        price_class_mapping,
        on=["Brand", "Category"],
        how="left",
    )
    updated_pillars = updated_pillars.rename(
        columns={
    ...
    updated_pillars["equity_pillar"] = (
        updated_pillars["equity_pillar"]
        .str.replace("_pillar_trend_past", "", regex=False)
        .str.replace("_", " ")
        .str.title()
    )
    updated_pillars = updated_pillars.sort_values(
        by=["brand", "category", "price_class", "equity_pillar"],
        ignore_index=True,
    )
    updated_pillars["relative_importance"] = pd.to_numeric(
        updated_pillars["relative_importance"], errors="coerce"
    ).round(4)
...
def updated_scorecard_format(
    ...
    summary_hd_price_renamed = merge_and_prepare_summary(
        detailed, df_long, price_class_mapping, dashboard_metric_names_mapping
    )
    updated_summary = clean_and_rename_summary(summary_hd_price_renamed)
    updated_summary = round_columns(updated_summary)
    updated_pillar_importances = prepare_pillar_importances(
        pillar_importances, price_class_mapping
    )
...
def scoring(
    ...
    eq_sub_scale_merged_brand["date"] = pd.to_datetime(
        eq_sub_scale_merged_brand["date"], utc=False
    )
    equity_dt["date"] = pd.to_datetime(equity_dt["date"], utc=False)

    processed_harmonized_data = pd.read_csv(
        output_config["processed_input_data"], storage_options=storage_options
    )
    processed_harmonized_data["date"] = pd.to_datetime(
        processed_harmonized_data["date"], utc=False
    )
    # output_file_path = f"{static_output_path}/dashboard_metric_names_mapping.csv"
    # dashboard_metric_names_mapping = pd.read_csv(
    #     mapping_config["dashboard_metric_names_mapping"],
    #     storage_options=storage_options,
    # )

    # output_file_path = f"{static_output_path}/price_class_mapping.csv"
    # price_class_mapping = pd.read_csv(
    #     mapping_config["price_class_mapping"], storage_options=storage_options
    # )
    ...
    if refresh_type == "model_refresh":
        ...
        weights_sheet.to_csv(
            output_config["pillar_creation"]["weights_sheet"],
            index=False,
            storage_options=storage_options,
        )

    if refresh_type == "model_scoring":
        weights_sheet = pd.read_csv(
            output_config["pillar_creation"]["weights_sheet"],
            storage_options=storage_options,
        )
    # Rename all columns to lowercase
    weights_sheet.columns = weights_sheet.columns.str.lower()
    eq_sub_scale_merged_brand_stacked = eq_sub_scale_merged_brand[
        eq_sub_scale_merged_brand["new_brand"] == "Stacked Brand"
    ]

    # eq_sub_scale_merged_brand_stacked["date"] = pd.to_datetime(eq_sub_scale_merged_brand_stacked["date"])
    # equity_dt["date"] = pd.to_datetime(equity_dt["date"])
    # eq_sub2 = pd.DataFrame()
    ...
    scaled_df = eq_sub_scale_merged_brand_stacked
    for category in weights_sheet["category"].unique():
        scaled_df_sub = (
            scaled_df[scaled_df["category"] == category]
            .copy()
            .reindex(sorted(scaled_df.columns), axis=1)
        )
        for pillar in weights_sheet["pillar"].unique():
            scaled_df_sub, index_df_long1 = create_pillar_scores(
                scaled_df_sub,
                weights_sheet,
                req_cols,
                category,
            ...
            index_df_long = pd.concat(
                [index_df_long, index_df_long1], ignore_index=True
            )
        index_df = pd.concat([index_df, scaled_df_sub], ignore_index=True)
    dv_data = equity_dt[
        [
            "date",
    ...
    index_df = pd.merge(
        index_df, dv_data, on=["date", "brand", "category"], how="left"
    )
    index_df_long = pd.merge(
        index_df_long, dv_data, on=["date", "brand", "category"], how="left"
    )

    index_df_long.to_csv(
        output_config["pillar_creation"]["pillars_long_format"],
        index=False,
        storage_options=storage_options,
    )
    index_df.to_csv(
        output_config["pillar_creation"]["pillars"],
        index=False,
        storage_options=storage_options,
    )
    ...
    for brand in index_df["brand"].unique():
        ...
        for category in brand_data["category"].unique():
            ...
            for pillar in (
                ...
                br_cat_df = pillar_trend_creation(
                    br_cat_df, brand, category, pillar, refresh_config
                )  # 3. done
            final_merged_df = pd.concat(
                [final_merged_df, br_cat_df], ignore_index=True
            )
    final_merged_df.to_csv(
        output_config["trend_pillar"]["trend_pillars"],
        index=False,
        storage_options=storage_options,
    )
    ...
    df1, grouped, pillar_columns, columns_to_mean, unique_combinations = (
        scaled_scores_prep(
    ...
    for _, group in unique_combinations.iterrows():
        ...
        processed_subset = compute_scaled_scores(
            ...
            pillar_columns,
            columns_to_mean,
            filter_config["scaled_score"]["only_pillars"],
        )  # 5. done
    ...
    columns_to_pivot = [
        col
        for col in scaled_scores.columns
        if col not in ["date", "brand", "category", refresh_config["dv"]]
    ]
    scaled_scores_long = pd.melt(
        scaled_scores,
        id_vars=["date", "brand", "category"],
        value_vars=columns_to_pivot,
        value_name="column_value",
    )

    scaled_scores.to_csv(
        output_config["scaled_scores"]["scaled_pillars"],
        index=False,
        storage_options=storage_options,
    )
    scaled_scores_long.to_csv(
        output_config["scaled_scores"]["scaled_pillars_long_format"],
        index=False,
        storage_options=storage_options,
    )
...
def post_modelling(
    ...
    nielsen_rms_data = pd.read_csv(
        output_config["processed_sales_data"], storage_options=storage_options
    )
    ...
    scaled_scores_long = pd.read_csv(
        output_config["scaled_scores"]["scaled_pillars_long_format"],
        storage_options=storage_options,
    )
    ...
    processed_harmonized_data["date"] = pd.to_datetime(
        processed_harmonized_data["date"]
    )
    price_class_mapping = pd.read_csv(
        mapping_config["price_class_mapping"], storage_options=storage_options
    )
    ...
    if (refresh_type == "model_refresh") | (
        (refresh_type == "model_scoring")
        & (refresh_config["run_importance_model_for_scoring_refresh"] == True)
    ):
        results_all_model = pd.DataFrame()
        for brand in final_merged_df.brand.unique():
            ...
            for category in br_df["category"].unique():
                ...
                results = pillar_importance_model(
                    final_merged_df,
                    nielsen_rms_data,
                    feat_eng_config,
                    refresh_config,
    ...
    if (refresh_type == "model_refresh") | (
        ...
        results_all_model.to_csv(
            output_config["importance_model"]["model_results"],
            index=False,
            storage_options=storage_options,
        )

    if (refresh_type == "model_scoring") & (
        refresh_config["run_importance_model_for_scoring_refresh"] == False
    ):
        results_all_model = pd.read_csv(
            output_config["importance_model"]["model_results"],
            storage_options=storage_options,
        )
    ...
    hub_data, pillar_importances, var_map = data_hub_data_format_code(
        pillars_long_data=index_df_long,
        weights_sheet=weights_sheet,
        scaled_scores_long=scaled_scores_long,
        imp_model_results=results_all_model,
        idv_list=req_cols,
        refresh_config=refresh_config,
    )

    hub_data.to_csv(
        output_config["pillar_importances"]["hub_data"],
        index=False,
        storage_options=storage_options,
    )
    pillar_importances.to_csv(
        output_config["pillar_importances"]["pillar_importances"],
        index=False,
        storage_options=storage_options,
    )
    var_map.to_csv(
        output_config["pillar_importances"]["variable_mapping"],
        index=False,
        storage_options=storage_options,
    )

    scorecard_detailed = hub_data.copy()
    scorecard_summary = scorecard_summary_creation(scorecard_detailed)

    hub_data.to_csv(
        output_config["scorecard"]["detailed"],
        index=False,
        storage_options=storage_options,
    )

    scorecard_summary.to_csv(
        output_config["scorecard"]["summary"],
        index=False,
        storage_options=storage_options,
    )

    pillar_importances.to_csv(
        output_config["scorecard"]["pillar_importances"],
        index=False,
        storage_options=storage_options,
    )

    updated_summary, updated_pillar_importances = updated_scorecard_format(
        scorecard_detailed,
        processed_harmonized_data,
        pillar_importances,
        dashboard_metric_names_mapping,
        price_class_mapping,
    )
    ...
    updated_summary.to_csv(
        output_config["updated_scorecard"]["updated_summary"],
        index=False,
        storage_options=storage_options,
    )
    updated_pillar_importances.to_csv(
        output_config["updated_scorecard"]["updated_pillar_importances"],
        index=False,
        storage_options=storage_options,
    )
    print("BHC refresh successfully completed")
